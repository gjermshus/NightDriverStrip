...
export function AreaStat({ classes, name, rawvalue, ignored, statsAnimateChange, maxSamples, headerFields, idleField, category, detail }: IAreaStatProps) {
    ...
    const [lastStates, setLastStates] = useState<any>([Object.entries(getChartValues(rawvalue)) // TODO: fix any type
        .filter(entry => !ignored.includes(entry[0]))
                ...
                {Object.entries(rawvalue)
                    ...
                    .map(entry =>
                        <ListItem className={classes.stats} key={entry[0]}>
                            <Typography color="textPrimary" variant="h5">{entry[0]}</Typography>:
                            <Typography color="textSecondary" variant="h5" >{getValue(entry[1])}</Typography>
                        </ListItem>)}
            </List>
        </Box>}
        <AreaChart
